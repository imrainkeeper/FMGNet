...
# 后续补充实验部分：
# try2: 修改了data_loader,与try1的区别在于try2的输入数据不再resize为1024x1024,而是resize为靠近8的整数倍(预测，一方面，
# resize到1024x1024会造成人头的形变可能会影响性能。但是另一方面，如果每张图片大小不同的话，网络的感受野在大图片上感受到的是局部，
# 在小图片上感受到的是全局信息，这样也有可能造成影响。)
#
# net16_local_try0 将trunk net中的concatenate改为sum
#####################################################
...
class net16_local(nn.Module):
    def __init__(self, load_weights=True):
        super(net16_local, self).__init__()
        ...
        self.trunk_backend2_1 = nn.Sequential(nn.Conv2d(512, 512, kernel_size=3, padding=1, stride=1),
                                              nn.ReLU(inplace=True))
        self.trunk_backend2_2 = nn.Sequential(nn.Conv2d(512, 512, kernel_size=3, padding=2, stride=1, dilation=2),
                                              nn.ReLU(inplace=True))
        self.trunk_backend3_1 = nn.Sequential(nn.Conv2d(512, 512, kernel_size=3, padding=1, stride=1),
                                              nn.ReLU(inplace=True))
        self.trunk_backend3_2 = nn.Sequential(nn.Conv2d(512, 512, kernel_size=3, padding=2, stride=1, dilation=2),
                                              nn.ReLU(inplace=True))
        self.trunk_backend3_3 = nn.Sequential(nn.Conv2d(512, 512, kernel_size=3, padding=4, stride=1, dilation=4),
                                              nn.ReLU(inplace=True))
        self.trunk_backend4_1 = nn.Sequential(nn.Conv2d(512, 512, kernel_size=3, padding=1, stride=1),
                                              nn.ReLU(inplace=True))
        self.trunk_backend4_2 = nn.Sequential(nn.Conv2d(512, 512, kernel_size=3, padding=2, stride=1, dilation=2),
                                              nn.ReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        x_trunk2 = x_trunk2_1 + x_trunk2_2                     # 采用sum的方式累加feature
        ...
        x_trunk3 = x_trunk3_1 + x_trunk3_2 + x_trunk3_3            # 采用sum的方式累加feature
        ...
        x_trunk4 = x_trunk4_1 + x_trunk4_2
