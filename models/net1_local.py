...
class single_SPPLayer(torch.nn.Module):  # 这里的single表示只进行一层分解，这和传统spp金字塔多层分解不同,此处可以算作是GAP的一个细化版
    def __init__(self, pool_type='avg_pool', split_coef=16):
...
class net1_local(nn.Module):
    def __init__(self, load_weights=True):
        super(net1_local, self).__init__()
        ...
        self.auxiliary_backend_output_layer = nn.Conv2d(64, 2, kernel_size=1)
