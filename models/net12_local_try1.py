...
# 由于在低级信息中加入了高层信息
# 2、 try0: gt mask map 和 x_share 相乘
#    try1: img 和 x_share 相乘（看是特征mask好还是图像mask好）
...
class net12_local(nn.Module):
    ...
    def forward(self, x, y):       # x是输入的图像，而y是gt_mask_map(大小为128x128)
        y_1024 = F.upsample_bilinear(y, size=(1024, 1024))
        x_after_mask = x * y_1024
        x_share = self.frontend(x_after_mask)
