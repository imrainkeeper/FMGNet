...
class trunk_net2_local(nn.Module):
    def __init__(self, load_weights=True):
        ...
        self.backend4_2 = nn.Sequential(nn.Conv2d(768, 256, kernel_size=3, padding=2, stride=1, dilation=2),
                                        nn.ReLU(inplace=True))
        self.backend4_3 = nn.Sequential(nn.Conv2d(768, 256, kernel_size=3, padding=4, stride=1, dilation=4),
                                        nn.ReLU(inplace=True))
        self.backend4_4 = nn.Sequential(nn.Conv2d(768, 256, kernel_size=3, padding=8, stride=1, dilation=8),
                                        nn.ReLU(inplace=True))
        self.output_layer = nn.Conv2d(1024, 1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x4_3 = self.backend4_3(x3)
        x4_4 = self.backend4_4(x3)
        x4 = torch.cat((x4_1, x4_2, x4_3, x4_4), 1)

        x_out = self.output_layer(x4)
        return x_out
