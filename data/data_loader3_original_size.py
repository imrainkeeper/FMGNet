###############################################
# 与data_loader3的区别在于data_loader3全部resize到1024x1024,而这个data_loader是resize到最近的8的整数倍(向上取整)
###############################################
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        new_width, new_height = int(np.ceil(width / 8) * 8), int(np.ceil(height / 8) * 8)
        ...
        gt_density_map = cv2.resize(gt_density_map, (int(new_width / 8), int(new_height / 8)), interpolation=cv2.INTER_CUBIC)
